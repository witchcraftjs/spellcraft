...
import { createContext } from "@witchcraft/shortcuts-manager"
...
export function useMultipleManagers(
	...
	const list = import.meta.client ? localStorage.getItem(managerNamesKey) : undefined
	...
	if (managerNames.value.length > 0) {
		...
		if (!atLeastOneOk) {
			const r = read("default", { force: true })
			notifyIfError(r)
			if (r.isOk) {
				load(r.value)
			}
	...
	function read(
		...
		if (!import.meta.client) return Result.Err(new Error(`Cannot read managers server side.`))
